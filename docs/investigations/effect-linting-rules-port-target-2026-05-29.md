...
- Several rules (such as `no-effect-ladder.grit`) enumerate 6 near-duplicate binding contexts (const / typed const / return x 1-2 args): a GritQL ergonomics limitation.
...
- **oxlint JS plugins are ALPHA (2026-03-11)**: https://oxc.rs/blog/2026-03-11-oxlint-js-plugins-alpha / https://oxc.rs/docs/guide/usage/linter/js-plugins. They use the **ESLint-compatible plugin API**, so an `eslint-plugin-*` package works in oxlint too, referenced by full npm name. **No prefix auto-resolution** and no `oxlint-plugin-` convention.
- **ast-grep**: no formal naming/distribution convention. Dominant model is a GitHub repo of YAML (`rules/<lang>/<cat>/<id>.yml`) consumed via `sgconfig.yml` path refs (such as `coderabbitai/ast-grep-essentials`, not on npm).
...
- Naming recommendation from agent: engine-neutral (such as `@scope/effect-lint`) if multi-engine future; `@scope/eslint-plugin-effect` if ESLint-first (gets auto-resolution). Engine-tied `oxlint-*` buys nothing (no registry).
...
- Source docs: docs.npmjs.com/trusted-publishers, /about-access-tokens, /cli/v11/commands/npm-publish. **Context**: the source pack (`@catenarycloud/linteffect`, commit `0854840`) already uses npm trusted-publisher OIDC: a real reference to copy.

---

## Investigator Findings: Section 1: PoC ports of 4 representative rules

**Environment / runner checks.** `which ast-grep sg oxlint vp node npx` found `ast-grep`, `sg`, `vp`, `node`, and `npx`; no global `oxlint` binary was installed. `npx --yes oxlint@latest --help` initially failed because `/Users/mp/.npm/_cacache` has permission-broken root-owned files, so I reran with `npm_config_cache=/tmp/linteffect-poc/npm-cache`. That succeeded within the one-minute timebox. PoC files were written only under `/tmp/linteffect-poc`; source repos under `/Users/mp/references/**` were read-only.
...
|---|---:|---|---|
| (a) Trivial single-node/member ban | 16 | Low effort: usually one `pattern` or a short `any` list; import guard can be added mechanically when present. | Low effort: one visitor (`CallExpression`, `ReturnStatement`, etc.) plus shared callee helpers. |
| (b) Syntax ban + Effect import guard | 4 | Low-to-moderate: target `kind` plus `inside: program` / `has: import` with `stopBy: end`; import matching needs a reusable util. | Low effort: collect imports in `Program`, report syntax-node visitors only when the file imports Effect/Atom. |
| (c) Relational / nested-contains | 19 | Moderate: mostly maps from Grit `contains` to `has` / `inside` with `stopBy: end`; requires AST-shape testing to avoid overmatching. | Low-to-moderate: imperative AST traversal is natural, but needs helper functions for callee names, ancestors, and nested calls. |
...
| `no-effect-fn-generator` | (a) | Specific call shape; direct `Effect.fn(function* ...)` patterns or visitor. |
...
| `no-effect-type-alias` | (c) | Type alias value contains `Effect.Effect`; type AST matching needed but direct. |
...
| `no-fromnullable-nullish-coalesce` | (a) | Direct `Option.fromNullable(x ?? null|undefined)` patterns. |
...
| `no-manual-effect-channels` | (a) | Type-reference ban for `Effect.Effect` / `Layer.Layer`; direct type-node visitor/pattern. |
...
| `no-match-void-branch` | (a) | Direct Match branch patterns. |
...
| `no-option-boolean-normalization` | (a) | Two concrete `Option.match` shapes; direct alternatives. |
...
| `no-return-null` | (a) | Return-literal syntax ban with import guard; direct. |
| `no-runtime-runfork` | (a) | Single member-call ban. |
| `no-string-sentinel-const` | (d) | Const string literal under Effect/Atom file guard; direct constraint, counted here because it constrains initializer shape. |
| `no-string-sentinel-return` | (a) | Direct `Effect.succeed("...")` pattern. |
...
| `prevent-dynamic-imports` | (a) | Single runtime-import ban, no Effect import gate. |
| `warn-effect-sync-wrapper` | (c) | `Effect.sync` callback wraps non-console side-effect call; needs negative console check. |

### Estimate correction

The earlier rough estimate was directionally right on hard-rule count but optimistic on ast-grep “low-effort” count. My corrected estimate from the actual 50-rule tree:

- **ast-grep:** about **20 low-effort** rules, with **25 moderate** and **5 hard**. The moderate count is higher because many Grit rules are not single-node bans; they are `contains`-based relational rules that need `stopBy: end`, constraints, and fixture validation.
- **oxlint JS plugins:** about **30 helper-aided low-effort** rules, with **15 moderate** and **5 hard**. This matches the oracle estimate closely. A small shared helper layer for imports, callee names, ancestor walking, and nested argument predicates turns many relational rules into short visitors.
...
**Verified from source and fixtures:** oxlint can load local `.ts` plugin files directly in supported runtimes. `oxc/apps/oxlint/src-js/package/config.generated.ts` documents both `"./custom-plugin.js"` and `"./custom-plugin.ts"`; it states TypeScript plugin files work natively in Deno/Bun and in Node.js `>=22.18.0` / `^20.19.0` because built-in type stripping is available. The local fixtures repeatedly use `.ts` directly, such as `oxc/apps/oxlint/test/fixtures/basic/.oxlintrc.json` has `"jsPlugins": ["./plugin.ts"]`, and `oxc/apps/oxlint/test/fixtures/js_config_js_plugins/oxlint.config.ts` has `jsPlugins: ["./plugin.ts"]`.
...
Observed result: first run created `rule-tests/__snapshots__/no-effect-as-snapshot.yml` and passed; second run passed with the snapshot present. This validates the expected npm distribution shape. The package ships YAML under `node_modules/<pkg>/rules`; consumers add that directory to `ruleDirs`; `sg test` uses `valid`/`invalid` snippets plus snapshots. The verified schema has no native `extends:` shorthand.
...
4. **Constrained metavars inside `not` need rewrites.** Grit rules like `no-return-in-arrow` express exclusions such as “do not report this return if the same return is inside `S.filter` / `Schema.filter`.” The safer ast-grep port shape matches the return node and adds ancestor/inside exclusions. Avoid reusing a captured `$ret` inside a nested negative relation. Treat this as an authoring hazard rather than a blocker.
...
The key realization: capability is NOT the deciding factor: **both engines expressed and executed all 4 representative rules (incl. the hard ones) against real fixtures**, and the other 46 categorize as feasible. The decision is workflow-fit + risk + distribution, where oxlint-as-ESLint-plugin wins for this user.
...
4. **The "helper library" cost is acceptable, not a mini-linter.** A small shared lib (callee-name normalization, import-guard, ancestor walk, nested-arg predicates) turns most relational rules into short visitors. Effort ≈ 30 low-effort / 15 moderate / 5 hard. For an agent-assisted port this is reviewable and testable.
...
- **Dual-runtime testing is real extra work.** De-risking via ESLint means writing ESLint `RuleTester` tests alongside oxlint fixture/snapshot tests. Worth it; not free.
...
- First-publish is low-effort in 2026 with **OIDC Trusted Publishing** (GA): no stored `NPM_TOKEN`. The source repo (`@catenarycloud/linteffect`, commit `0854840`) already demonstrates the OIDC trusted-publisher setup: copy it.
...
- Repo name can match the unscoped package (such as repo `eslint-plugin-effect-style`).
...
> Prose note: at the time of the original investigation, this internal artifact was not prose-linted because it was dense with technical/code identifiers (oxlint, ast-grep, GritQL, metavariable, Effect APIs). Current prose policy lives in `docs/references/prose-gate.md`.
...
- **`kevinmichaelchen/biome-effect-linting-rules` @ branch `feat/full-parity-and-coverage`** (single commit `30a8072`, 2026-04-08; **1 ahead / 16 behind** upstream master).
  ...
  - **Staleness/gaps**: the fork uses a single squashed commit and has been untouched for ~7 weeks. It is older than the 2 newest rules. The missing rules are `no-naked-object-state-update` (Apr 18) and `no-family-collection-read` (May 13). Expected coverage is ~48 of 50 rules. Bundles lintcn (Go), which is out of scope for us.
...
- **KEY INSIGHT: Rika's rules are a DIFFERENT AXIS than linteffect.** linteffect (50) = composition/control-flow SHAPE (flatten ladders, no nested gen, no if/switch/ternary, no pipe-ladder). Rika effect-* = SEMANTIC / ARCHITECTURAL / runtime / error-model hygiene. They mostly complement linteffect instead of duplicating it.
...
- `@rikalabs/oxlint-standards` **is published to npm**: v**0.8.1** (modified 2026-04-18), ships compiled `dist/` + types, exports `.` (config/presets entry), `./plugin` (the oxlint JS plugin), `./presets/*`. It's a real installable oxlint plugin, NOT just a GitHub repo.
...
- **So: depend + enable, don't copy.** For Rika's effect rules, the correct move is `pnpm add -D @rikalabs/oxlint-standards`, add its plugin to `jsPlugins`, enable the `effect-*` rules/presets you want. Copying defeats the point of a published plugin.
...
- **ARCHITECTURE REFRAME (the real answer)**: split deliverables.
  1. **Author + publish only what nobody else has**: the linteffect *composition-shape* rules ported to oxlint (net-new: Rika doesn't cover this axis; the kevinmichaelchen fork is unpublished/stale). This is the one piece worth building.
...
- Why it's bad: layer memoization is scoped per-`provide` call (reference-equality within one scope). Separate provides → a shared dependency (such as `ConfigLive` used by both A and B) is built **N times**, with separate `Scope` lifecycles/resources.
...
- Shape: structural, no type info: detect 2+ `Effect.provide` member-calls within one pipe chain. Low-to-medium port.
